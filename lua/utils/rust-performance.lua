...
function M.get_delta_status()
  ...
  if use_delta then
    ...
    local version_result = vim.system({ "delta", "--version" }, { text = true }):wait(5000)
    local version = vim.trim(version_result.stdout or "unknown")
...
function M.build_blink_rust()
  ...
  print(icons.status.sync .. " Compiling Blink.cmp Rust binary (async)...")

  -- Async build to avoid blocking the UI
  local build_cmd = string.format("cd %s && cargo build --profile ultra 2>&1", vim.fn.shellescape(blink_path))

  vim.fn.jobstart({ "/bin/sh", "-c", build_cmd }, {
    ...
    on_exit = function(_, exit_code)
      vim.schedule(function()
        if exit_code == 0 then
          ...
          print(icons.status.error .. " Compilation failed. Run manually: cd " .. blink_path .. " && cargo build --profile ultra")
        end
      end)
    end,
  })
...
function M.benchmark_fuzzy_performance()
  ...
  print(icons.performance.benchmark .. " Benchmark running (async)...")

  -- Simple test with fzf vs grep
  local test_cmd = [[hyperfine --warmup 3 'echo "test\nother\nstring\nmatch" | fzf -f "te"' 'echo "test\nother\nstring\nmatch" | grep "te"' --export-markdown /tmp/fuzzy_benchmark.md]]

  vim.fn.jobstart({ "/bin/sh", "-c", test_cmd }, {
    on_exit = function(_, exit_code)
      vim.schedule(function()
        if exit_code == 0 and vim.fn.filereadable("/tmp/fuzzy_benchmark.md") == 1 then
          print(icons.status.success .. " Benchmark completed - see /tmp/fuzzy_benchmark.md")
        else
          print(icons.status.error .. " Benchmark failed (exit code: " .. exit_code .. ")")
        end
      end)
    end,
  })
...
function M.setup_cross_compilation()
  ...
  local result = vim.system({ "rustup", "target", "list", "--installed" }, { text = true }):wait(10000)
  local installed_list = result.code == 0 and (result.stdout or ""):gsub("\n", " ") or ""
...
function M.ultimate_benchmark()
  ...
  local targets_result = vim.system({ "rustup", "target", "list", "--installed" }, { text = true }):wait(10000)
  local installed_targets_list = targets_result.code == 0 and (targets_result.stdout or ""):gsub("\n", " ") or ""
