...
local health_checks = {
  ts_ls = function()
    ...
    if not cmd_ok then
      return false, "typescript-language-server nicht gefunden. Installation: npm install -g typescript-language-server"
    end
  ...
  htmlls = function()
    ...
    if not cmd_ok then
      return false, "vscode-html-language-server nicht gefunden. Installation: npm install -g vscode-langservers-extracted"
    end
  ...
  cssls = function()
    ...
    if not cmd_ok then
      return false, "vscode-css-language-server nicht gefunden. Installation: npm install -g vscode-langservers-extracted"
    end
  ...
  jsonls = function()
    ...
    if not cmd_ok then
      return false, "vscode-json-language-server nicht gefunden. Installation: npm install -g vscode-langservers-extracted"
    end
  ...
  rust_analyzer = function()
    ...
    if not rustc_ok then
      return false, "Rust toolchain nicht gefunden. Installation: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh"
    end
  ...
  luals = function()
    ...
    if not cmd_ok then
      return false, "lua-language-server nicht gefunden. Installation: pacman -S lua-language-server"
    end
